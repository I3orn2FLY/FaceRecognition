...
class MtcnnDetector(object):
    ...
    def detect_faces_helper(self, img, det_type=0):
        TAG = "detect_faces_helper|"
        """
    ...
    def detect_faces(self, frame, min_fr=0.05):
        ret = self.detect_faces_helper(frame)
        # print(ret)
        if ret is None:
            return None
        face_box, face_points = ret

        if face_box.shape[0] == 0:
            return None
        ...
        for face in range(0, face_box.shape[0]):

            bbox = face_box[face, 0:4]
            width = bbox[2] - bbox[0]
            height = bbox[3] - bbox[1]

            if min_fr > float(height) / frame.shape[0]:
                continue

            boxes.append(bbox)

            points = face_points[face, :].reshape((2, 5)).T.astype(np.int)

            nimg, align_coeff = face_preprocess.preprocess(frame, bbox, points, image_size='112,112')
            nimg = cv2.cvtColor(nimg, cv2.COLOR_BGR2RGB)
            aligned = np.transpose(nimg, (2, 0, 1))

            faces.append(aligned)

            align_coeffs.append(align_coeff)

            landmark = points - bbox[:2]
            landmark = landmark / np.array([width, height])
            landmarks_list.append(landmark)

        if not faces:
            return None

        return self.model.get_feature2(faces), boxes, align_coeffs, landmarks_list
...
class DetResult():
    ...
    def addFace(self, fvec, bbox, criterion, landmarks):
        bbox[0] -= self.pivot
        bbox[2] -= self.pivot

        self.fvecs.append(fvec)
        self.boxes.append(bbox)
        self.criterions.append(criterion)
        self.landmarks_list.append(landmarks)

    def isEmpty(self):
        return len(self.fvecs) == 0

    def getResults(self):
        return self.fvecs, self.boxes, self.criterions, self.landmarks_list
